...
tiltom <- function(omelt,plus=0.02){
...
gseqsix <- mclapply(rescan.calls, eval, mc.cores=min(50L,detectCores()))
gscan.eqs <- sapply(gseqsix,function(x) unlist(x[-101]))
dim(gscan.eqs) <- c(19,100,10,18)
gsix.means <- apply(gscan.eqs,c(1,4),mean)
gsix.upper <- apply(gscan.eqs,c(1,4),quantile,probs=0.975)
gsix.lower <- apply(gscan.eqs,c(1,4),quantile,probs=0.025)
save(gsix.means,gsix.upper,gsix.lower,file="gseqsix.RData")
if (!interactive()) q("no")
