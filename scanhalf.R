...
gseqhalf <- mclapply(rescan.calls, eval, mc.cores=min(50L,detectCores()))
gscan.eqs <- sapply(gseqhalf,function(x) unlist(x[-101]))
dim(gscan.eqs) <- c(19,100,10,18)
gscan.means <- apply(gscan.eqs,c(1,4),mean)
gscan.upper <- apply(gscan.eqs,c(1,4),quantile,probs=0.975)
gscan.lower <- apply(gscan.eqs,c(1,4),quantile,probs=0.025)
save(gscan.means,gscan.upper,gscan.lower,file="gseqhalf.RData")
if (!interactive()) q("no")
